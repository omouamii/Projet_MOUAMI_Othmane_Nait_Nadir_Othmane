// ex5_display.c — Arrêt/redémarrage (destroy) + affichage comme la capture
// Compile: gcc ex5_display.c -o ex5 -lvirt
// Usage  : ./ex5 [nom_domaine] [timeout_s]   (defaults: vm10, 60)
...
static int wait_until_state(virDomainPtr dom, int targetState, int timeout_sec) {
    for (int i = 0; i < timeout_sec * 2; i++) {           // demi-secondes
...
/* ====== AFFICHAGE (mimique de la capture) ====== */
...
static void print_active_domains(virConnectPtr conn) {
    ...
    for (int i = 0; i < got; i++) {
        ...
        printf("   state : %d\n", di.state);
        printf("   maxMem : %llu\n", (unsigned long long)di.maxMem);
        printf("   memory : %llu\n", (unsigned long long)di.memory);
        printf("   nrVirtCpu : %u\n", di.nrVirtCpu);
        printf("   cpuTime : %llu\n", (unsigned long long)di.cpuTime);
...
int main(int argc, char **argv) {
    ...
    if (!conn) {
        fprintf(stderr, "Impossible de se connecter à libvirt\n");
        return 1;
    }

    /* En-tête + état initial (comme la capture) */
    print_header(conn);
    ...
    /* === ARRET (destroy) === */
    printf("\n>>> Arret de %s\n", domName);
    if (virDomainDestroyFlags(dom, VIR_DOMAIN_DESTROY_GRACEFUL) < 0) {
